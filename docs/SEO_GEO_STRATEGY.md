...
## Phase 4.5: カテゴリ体系 v2 + ブログUX ✅ 実装済み（2026-07-03）

公開11記事の実態（「保護者向け」が機能せず、AI系記事が最大クラスタ化）を受けて再設計。

### カテゴリ体系 v2（Notion の Category と `lib/blogTaxonomy.ts` を必ず一致させる）

| カテゴリ | slug | 役割 |
|---|---|---|
| 子育て | kosodate | 悩み・家庭でのかかわり方・不登校（悩み検索からの入口） |
| AI・ロボット時代の学び | ai-robot | 最大クラスタ・GEO主戦場 |
| 探究・対話の実践 | tankyu-taiwa | Yononaka/PBL現場の一次情報（E-E-A-T源） |
| 教育を考える | kyoiku | 思想・ブランド（なぜCLAFTか） |
| 習い事・スクール選び | naraigoto | CV直結。**最優先で記事補充**（現状1本のみ） |

運用ルール: 1記事1カテゴリ厳守／各カテゴリに Pillar（柱）を1本指定し枝から内部リンク／
横断テーマ（Yononaka・マイクラ等）は Tags で拾う／旧カテゴリ（探究学習・キャリア教育・
オンライン学習・プログラミング・保護者向け）は廃止。

### ブログUX・ナビゲーション（実装済み）
- **カテゴリページ** `/blog/category/[slug]`（メタデータ・パンくずJSON-LD付き。テーマの専門棚として
  インデックスさせる）。カテゴリチップは一覧・記事ともリンク化。
- **月別アーカイブ** `/blog/archive/[YYYY-MM]`
- **記事検索**（一覧ページのキーワード絞り込み・クライアントサイド）
- **関連記事**（記事末に同カテゴリ最新3件）
- **RSSフィード** `/blog/feed.xml`（リーダー・AIクローラ向け）
- sitemap にカテゴリ（記事のあるもののみ）・月別を自動追加

### 読者フィードバック（実装済み・段階導入）
1. ✅ 記事末「役に立った？👍👎」→ GA4 `article_feedback`（リライト対象の発見器）
2. ✅ 記事末「質問・感想フォーム」→ FormSubmit（keeponlabo@gmail.com）＋GA4 `blog_question_submit`。
   **届いた質問は記事にFAQとして追記（FAQPage構造化データ化）→ GEOのQ&A供給源**。次の記事ネタにも。
3. ⏳ 承認制コメント欄は読者が育ってから（展示会プラットフォームの Supabase コメント基盤を転用予定）

### 月次レビューに追加する項目
- GA4 `article_feedback`（👎が多い記事をリライト）／`blog_question_submit`（質問→FAQ追記→GEO強化）

## Phase 5: オフサイト被引用づくり + 月次改善ループ

検索とAIの両方で「権威性」を積み上げ、書いた記事を成果（体験予約）につなげる運用フェーズ。

### A. 被引用づくり（オフサイト / GEOの土台）
AIは「他所でどれだけ言及・参照されているか」を信頼の手がかりにする。CLAFTへの外部言及を増やす:
1. **Googleビジネスプロフィール（八尾教室）** を整備＋口コミ依頼。LocalBusiness構造化データと連動し、
   ローカル検索・地図・AIのローカル回答に効く。最優先・無料。
2. **プレスリリース**（PR TIMES等）で「オンラインフリースクール化」を発信 → ニュース被リンク＆AI参照源化。
3. **比較・まとめ系への掲載**（習い事比較、フリースクール一覧、探究スクールまとめ）。AIは「比較・一覧」
   ページを回答時に多用するため、そこに載ること自体が露出になる。
4. **note / YouTube / Instagram** と記事の相互リンク。プロフィールに公式サイトを明記し `sameAs` を一貫させる
   （エンティティの一貫性）。代表・将一郎の実名露出で E-E-A-T を補強。
5. 教育系メディア・地域メディアへの寄稿。

### B. 月次改善ループ（測って、伸ばす）
毎月この順でレビュー:
1. **GSC**: 表示回数・CTR・掲載順位を確認。伸びているクエリには記事を追加、CTRが低いページはタイトル/
   ディスクリプションを改善。新記事は公開後にURL検査でインデックス促進。
2. **GA4**: `/blog` の流入、AI参照元セグメント（`chatgpt.com`/`perplexity.ai`/`gemini.google.com`/
   `copilot`）、`reservation_submit`（CV）への寄与を確認。
3. **GEO順位チェック（重要）**: ChatGPT / Perplexity / Gemini に想定質問（例「塾以外で子どもの考える力を
   伸ばす習い事は？」）を実際に打ち込み、**CLAFTが引用・推薦されるか**を毎月記録。されなければ該当記事を
   一次情報・比較表で強化。これがGEOの“順位測定”。
4. **コンテンツ運用**: 古い記事は `PublishedAt`/本文を更新（dateModified更新）、内部リンクを追加、
   クラスターを拡張。薄い重複記事は統合（カニバリ回避）。

### C. KPI
- 主KPI: **無料体験・個別相談の予約数**（GA4 `reservation_submit`）。
- 補助: 公開記事数、主要クエリの表示回数/順位、AI被引用回数、オーガニック流入数。

### ブログ運用メモ（記事制作）
記事は Notion DB「CLAFT Blog（SEO/GEO記事）」で制作。既存: `juku-tankyu-chigai`（柱）/
`juku-igai-kangaeru-chikara`（柱）/ `pbl-theme-ai-shitsumon`（枝）。新記事は既存と検索意図が
**被らない切り口**にし、柱へ内部リンクする（キーワードカニバリ回避）。制作は別Claude＋オープン
クエスチョン方式（前提プロンプトは別途用意）。
